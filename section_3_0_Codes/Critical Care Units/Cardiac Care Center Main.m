%% Load weekly data
drugFree = readtable('Cardiac_Care_Unit_DrugUsers_weekly_summary.csv');
drugUsers = readtable('Cardiac_Care_Unit_DrugFree_weekly_summary.csv');
...
%% Save .mat results
save('Cardic_Care_Center.mat', 'results', 'chain', 's2chain');

%% Save chain as CSV
chain_table = array2table(chain, 'VariableNames', results.names);
chain_table.sigma2 = s2chain;
writetable(chain_table, 'cardic_care_unit.csv');
