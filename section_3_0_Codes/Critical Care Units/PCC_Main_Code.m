%% Load weekly data
drugFree = readtable('Progressive_Care_Unit_DrugUsers_weekly_summary.csv');
drugUsers = readtable('Progressive_Care_Unit_DrugFree_weekly_summary.csv');
...
%% Save .mat results
save('PCC_Main_Code.mat', 'results', 'chain', 's2chain');

%% Save chain as CSV
chain_table = array2table(chain, 'VariableNames', results.names);
chain_table.sigma2 = s2chain;
writetable(chain_table, 'PCC_Main_Code_chain.csv');
