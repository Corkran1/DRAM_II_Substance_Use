%% Load weekly data
%% fit_mrsa_dram.m
% DRAM MCMC to fit MRSA transmission model and save results to CSV

clear; clc;

%% Load weekly data
drugFree = readtable('Inpatient_3_Blue_DrugUsers_weekly_summary.csv');
drugUsers = readtable('Inpatient_3_Blue_DrugFree_weekly_summary.csv');
...
%% Save .mat results
save('Inpatient_3_Blue.mat', 'results', 'chain', 's2chain');

%% Save chain as CSV
chain_table = array2table(chain, 'VariableNames', results.names);
chain_table.sigma2 = s2chain;
writetable(chain_table, 'Inpatient_3_Blue_chain.csv');

%% Plot diagnostics
mcmc_diagnostics_plot(chain, results);

disp('MCMC run and CSV export complete.');
