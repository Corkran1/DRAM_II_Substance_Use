%% Load weekly data
drugFree = readtable('Inpatient_4_Green_DrugUsers_weekly_summary.csv');
drugUsers = readtable('Inpatient_4_Green_DrugFree_weekly_summary.csv');
...
%% Save .mat results
save('Inpatient_4_Green.mat', 'results', 'chain', 's2chain');

%% Save chain as CSV
chain_table = array2table(chain, 'VariableNames', results.names);
chain_table.sigma2 = s2chain;
writetable(chain_table, 'Inpatient_4_Green_chain.csv');
