%% Load weekly data
drugFree = readtable('UHTMC_CT_DrugFree_weekly_summary.csv');
drugUsers = readtable('UHTMC_CT_DrugUsers_weekly_summary.csv');
...
%% Save .mat results
save('CT_Department.mat', 'results', 'chain', 's2chain');

%% Save chain as CSV
chain_table = array2table(chain, 'VariableNames', results.names);
chain_table.sigma2 = s2chain;
writetable(chain_table, 'CT_Department_Main_chain.csv');
